...
## Week 4 — PriorityQueue

- 學到什麼：
  - 跟 `Queue<T>` 的差異：FIFO 由「到達時間」決定順序，Priority Queue 改成由「優先權」決定順序；`dequeue()` 完全不用改（永遠拿最前面），複雜度全部轉移到 `enqueue()`——新資料要插到正確位置，而不是永遠塞到尾端
  - `items` 陣列要維持「優先權由高到低排序」的不變量，`enqueue()` 的責任就是在插入新元素時不破壞這個不變量
  - 插入演算法：預設插入位置＝陣列尾端；從頭掃描既有元素，找到第一個 `priority` 比自己**嚴格小於**的位置，記下來就 `break`；用 `splice(insertIndex, 0, 新元素)` 插入
  - 為什麼比較條件要用 `<` 而不是 `<=`：`<` 會跳過所有優先權相同的既有元素，一路找到第一個比自己低的才停，於是新元素會被插在「同優先權群組」的最後面，維持「同優先權內先進先出」；如果改成 `<=`，掃到第一個相同優先權的就會停下來，新元素反而插到該優先權群組最前面，變成後到插隊到先到前面
- 弄壞了什麼、怎麼弄壞的：
  - 沒有寫錯，這次是全新的演算法（不是之前重複用的「存 resolve 排隊」套路），第一次自己寫不出來，經過導師完整拆解三步驟（預設尾端、掃描找插入點、splice 插入）跟具體 trace 之後，一次寫對
- 怎麼修好的：
  - （不適用，沒有 bug，是理解上需要新的鷹架）
- 還沒搞懂的地方：
  - （持續更新）
