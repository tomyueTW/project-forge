...
**你的回答：** 互斥鎖：同一時間，只有一個「持有鎖的人」可以執行臨界區的程式碼，其他想進臨界區的人，必須排隊等，直到目前持有鎖的人「釋放」鎖。`SafeCounter` 消除讓出執行權的空檔，`LockedCounter` 保護臨界區不被插隊。

**Review：** 對。這句話就是這兩種手段的精確區分：一個是「消除問題發生的條件」，一個是「允許條件發生，但保護受影響的資源」。
...
**你的回答：** 因為如果大部分的時間都在讀取的話，其實可以讓所有的讀取連線都直接讀取，不需要刻意等待。RWLock 允許多個 reader 同時持有，只有 writer 需要獨佔。

**Review：** 通過。核心差異（reader 可並存、writer 需獨佔）跟為什麼讀多寫少場景用 Mutex 浪費，兩個重點都講到了。
...
**你的回答：** 已經被叫醒、早就讀完的舊 reader，被重複算了一次。

**Review：** 通過，抓到了核心（舊 reader 被重複算）。補完整條因果鏈：`forEach` 只拜訪、不移除陣列內容，`shift()` 才會真的清空。不清空的話，下次觸發同一分支時，早就叫醒過、早就讀完、早就呼叫過 `releaseRead()` 的舊 resolve 函式會被再呼叫一次——`next()` 對已 resolve 的 Promise 呼叫不會報錯，但 `this.activeReaders++` 沒有這層保護，照樣執行，`activeReaders` 被持續灌水，永遠回不到 0，`acquireWrite()` 的 `activeReaders === 0` 永遠不成立，writer 永久卡死。
...
## PriorityQueue（Week 4）

## Q22. 為什麼 `enqueue()` 掃描既有元素時，用的條件是嚴格小於（`priority < priority`），而不是 `<=`？如果改成 `<=` 會發生什麼問題？
**你的回答：** 如果改成 `<=`，同優先權會插到前面，破壞先進先出。

**Review：** 通過，而且講得精準。用 `<=` 的話，掃描到第一個「相同或更低」優先權的元素就會停下來——而同優先權群組裡最先被掃到的一定是最早塞進去的那個，所以新元素會被插在整個同優先權群組的**最前面**，變成後到的插隊到先到前面，完全反過來，破壞了「同優先權內先進先出」這個 tie-break 規則。

---

## Cancellation（Week 4）

## Q23. 取消時為什麼要用 `indexOf()` 找位置、`splice()` 刪除，而不能直接用 `shift()`？另外為什麼要先判斷 `index !== -1` 才執行刪除跟 `reject()`？
**你的回答：** （第一部分）因為要找到取消的那筆資料。（第二部分，經過提示 `splice(-1, 1)` 的行為後）因為可能找不到該目標，所以要檢查 `index !== -1` 才不會誤刪最後一筆資料。

**Review：** 通過。兩部分都對：`shift()` 永遠拿最前面那個，取消的不一定排最前面，必須用 `indexOf` 精準定位、`splice` 刪除指定位置。`index !== -1` 的檢查是因為存在競態——如果 `enqueue()` 搶先一步已經正常處理掉這個 consumer，`waitingConsumers` 裡就找不到它，`indexOf` 回傳 `-1`；這時候如果沒檢查、直接 `splice(-1, 1)`，因為負數 index 代表「從陣列尾端數過來」，會誤刪陣列**最後一個、完全無辜的其他 consumer**，讓一個原本不相干的等待者被永久踢出佇列、卡死——這是「忘記防呆反而製造新 bug」的具體案例。
