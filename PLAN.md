...
**每個新 primitive／概念，三個步驟要分開、依序進行，不要混在同一則訊息裡：**

```text
1. 教學   —— 先把概念、動機、機制講完整（可以用比喻、圖、trace），
              這階段不出骨架、不叫你動手，先確認你聽懂了
2. 實作   —— 概念確認後，才給骨架程式碼，你動手把 TODO 補完
3. 回顧   —— 跑起來、typecheck、review 程式碼，討論踩到的坑
4. 記錄   —— 每個 primitive 做完，當場出面試考題、你作答、我 review，
              並且更新 learning-log.md ——不要等到整個 Week 結束才一次補
```

**為什麼要分開**：教學跟骨架擠在同一則訊息裡，容易變成「講完概念立刻丟程式碼」，你還沒消化完概念就要動手，會卡在「不知道從哪下手」。分成明確階段，你可以在教學階段就先問清楚，確認懂了再動手，效率更高。「記錄」也不要拖到週末才補——每個 primitive 做完當下記憶最新鮮，考題跟學習紀錄要當場做完。
